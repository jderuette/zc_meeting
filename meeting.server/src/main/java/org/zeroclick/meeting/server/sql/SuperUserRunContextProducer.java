...
public class SuperUserRunContextProducer extends ServerRunContextProducer {
	...
	protected void initSuperUserSubject() {
		this.subject.set(CONFIG.getPropertyValue(SuperUserSubjectProperty.class));
	}
	...
	public ServerRunContext produce() {
		final ServerRunContext superUserRunContext = ServerRunContexts.empty()
				.withRunMonitor(BEANS.get(RunMonitor.class)).withUserAgent(UserAgents.createDefault())
				.withSubject(this.subject.get());
	...
	public IServerSession getSession() {
		return this.produce().getSession(); // via provide(), because the
											// session is created on first
											// usage.
	}
