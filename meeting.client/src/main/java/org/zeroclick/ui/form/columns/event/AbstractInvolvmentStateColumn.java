 ...
 *
 */
public abstract class AbstractInvolvmentStateColumn extends AbstractSmartColumn<String> {
	// In User context, so texts are translated
	final InvolvmentStateCodeType involvmentStateCodes = new InvolvmentStateCodeType();
	...
	protected void execDecorateCell(final Cell cell, final ITableRow row) {
		...
		if (null != stateValue) {
			...
			final ICode<String> currentStateCode = this.involvmentStateCodes.getCode(stateColumnValue);
